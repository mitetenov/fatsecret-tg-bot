...
| Кода нет в базе FatSecret | ✅ Open Food Facts → модель с веб-поиском → чтение этикетки |
...
| КБЖУ жидкостей на 100 мл | ✅ не смешиваются со значениями на 100 г |
| Сомнительное распознавание | ✅ показывает confidence и требует повторного подтверждения |
| Кириллица в базе продуктов | ❌ датасет US/английский, поэтому нужен слой перевода |

Поиск использует актуальные методы FatSecret `foods.search.v5`, `food.get.v5`,
`food.find_id_for_barcode.v2`, `foods.autocomplete.v2` и
`foods.get_recently_eaten.v2`. Детальные порции из search/barcode переиспользуются,
чтобы не делать отдельный `food.get` для каждого кандидата.
