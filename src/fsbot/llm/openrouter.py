...
пищевой ценности.
Верни JSON: {"kind":"label","items":[{...}]} с одним объектом: query_en (бренд и
название по-английски для поиска в базе продуктов), name_ru (как написано на упаковке),
amount (100), unit ("g").
Отвечай только JSON, без пояснений."""

CLASSIFY_PROMPT = """На фото еда или упаковка продукта?
Ответь одним словом: plate — если приготовленная еда, напиток или блюдо;
label — если упаковка, этикетка или таблица пищевой ценности."""
